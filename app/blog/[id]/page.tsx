...
export default function BlogPage({ params }: { params: BlogParams }) {
    ...
    return (
        <section className="flex flex-col md:flex-row gap-5 p-5 w-full h-full max-w-6xl pt-20 pb-32">
            <header className="relative flex flex-col md:w-1/3 justify-between items-center md:items-start">
                <div className="text-center md:text-left md:w-96 md:fixed md:px-4">
                    ...
                    <Button
                        ...
                        as={Link}
                        href="https://wa.me/+56940006843?text=Hola!%20Me%20gustaria%20hacer%20una%20reserva.%20Tenes%20alguna%20reservacion%20disponible?%20Muchas%20gracias!😊"
                        color="secondary"
                        variant="shadow"
            ...
            <article className="md:w-2/3 text-lg md:text-left text-white">
                ...
                {content.map((item, index) => (
                    <div key={index} className="my-5">
                        ...
                        <p className="text-lg text-center text-pretty indent-5 md:text-left">{item.text}</p>
                    </div>
                ))}
                <div className="my-5">
                    <h4 className="text-2xl text-center font-bold pb-2 text-secondary md:text-left">Conclusion</h4>
                    <p className="text-lg text-center text-pretty indent-5 md:text-left">{conclusion}</p>
                </div>
            </article>
        </section>
    );
